...
describe('Perception Log Creation', () => {
  it('should write a perception log with primary_emotion as a normalized string', async () => {
    const response = await fetch(`${API_BASE}/api/create-perception-log`, {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        // Send emotion as OPM v4 object — should be normalized to string
    ...
    // May fail with 500 if test UUIDs don't exist in DB — that's expected in CI
    // The key check is that it doesn't crash AND the emotion format is correct
    if (response.ok) {
      const data = await response.json()
      expect(data.log).toBeTruthy()
      // primary_emotion should be a plain string, not a JSON object
      ...
      // Canon context should be present
      expect(data.canon).toBeTruthy()
      expect(typeof data.canon.phase).toBe('string')
    } else {
      // Log the error but don't necessarily fail — DB constraints may reject test UUIDs
